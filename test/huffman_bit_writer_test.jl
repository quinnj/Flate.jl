...
mutable struct huffTest
    input::String
    want::String
    wantNoInput::String
    tokens::Go.Slice{Flate.Token}
end

huffTest(input::String) = huffTest(input, "", "", Go.Slice(Flate.Token, 0))

#  testWriterEOF tests if the written block contains an EOF marker.
function testWriterEOF(ttype::String, test::huffTest, useInput::Bool)
    if useInput && test.input == ""
        return
    end
    input = Go.Slice(UInt8, 0)
    if useInput
        input = Go.Slice(read(test.input))
    end
    buf = IOBuffer()
    bw = Flate.newHuffmanBitWriter(buf)
    if ttype == "wb"
        Flate.writeBlock(bw, test.tokens, true, input)
    elseif ttype == "dyn"
        Flate.writeBlockDynamic(bw, test.tokens, true, input)
    elseif ttype == "huff"
        Flate.writeBlockHuff(bw, true, input)
    else
...
function testBlockHuff(in::String, out::String)
    @info("Testing $in")
    all = Go.Slice(read(in))
    buf = IOBuffer()
    bw = Flate.newHuffmanBitWriter(buf)
    w = bw
    ...
    @test got == want
    @info("Reset ok")
    testWriterEOF("huff", huffTest(in), true)
end

@testset "TestBlockHuff" begin
    #  determine input files
    for in in readdir(joinpath(dirname(pathof(Flate)), "../test/testdata/"); join=true)
        (contains(in, "huffman-") && endswith(in, ".in")) || continue
        #  for files where input and output are identical
        out = splitext(in)[1] * ".golden"
        testBlockHuff(in, out)
    end
end
...
function testBlock(test::huffTest, ttype::String)
    ...
    if test.input != ""
        ...
        @test got == want
        @info("Reset ok")
        testWriterEOF("wb", test, true)
    end

    @info("Testing $(twantNoInput)")
    wantNI = Go.Slice(read(twantNoInput))
    buf = IOBuffer()
    ...
    @test got == wantNI
    @info("Reset ok")
    testWriterEOF("wb", test, false)
end

function writeToType(
    ttype::String,
    bw::Flate.HuffmanBitWriter,
    tok::Go.Slice{Flate.Token},
    input::Go.Slice{UInt8},
)
    if ttype == "wb"
        Flate.writeBlock(bw, tok, false, input)
    elseif ttype == "dyn"
        Flate.writeBlockDynamic(bw, tok, false, input)
    else
        error("unknown test type")
    end
    flush(bw)
